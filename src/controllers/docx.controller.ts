...
import IDocx, { IDocxStamp } from "../types/IDocx.interface"
...
class DocxController {
  ...
  private addStampToPdf = async ({
    ...
    for (const stamp of stamps) {
      if (stamp.pageNum < 1 || stamp.pageNum > pages.length) {
        console.warn(`Страница ${stamp.pageNum} не существует в документе`)
        continue
      }

      const page = pages[stamp.pageNum - 1]
      const { width, height } = page.getSize()

      let x: number
      switch (stamp.horizontal) {
        case "left":
          x = stamp.offsetX || 40
          break
        case "right":
          x = width - stampDims.width - (stamp.offsetX || 40)
          break
        case "center":
          x = (width - stampDims.width) / 2 + (stamp.offsetX || 0)
          break
      }

      let y: number
      switch (stamp.vertical) {
        case "bottom":
          y = stamp.offsetY
          break
        case "top":
          y = height - stampDims.height - stamp.offsetY
          break
        case "center":
          y = (height - stampDims.height) / 2 + stamp.offsetY
          break
      }

      if (
        x < 0 ||
        x > width - stampDims.width ||
        y < 0 ||
        y > height - stampDims.height
      ) {
        console.warn(`Штамп выходит за пределы страницы ${stamp.pageNum}`)
        continue
      }
    ...
    // console.log(`Печати добавлены в ${outputPdfPath}`);
  }
  ...
  public saveDocx = async (
    ...
    const {
      file_key,
      file_type,
    }: { file_key: IDocx["file_key"]; file_type: IDocx["file_type"] } = req.body
  ...
  public testDocx = async (
    ...
    try {
      ...
      await this.addStampToPdf({
        ...
        stamps: docxFields[file_type].stamps,
      })
